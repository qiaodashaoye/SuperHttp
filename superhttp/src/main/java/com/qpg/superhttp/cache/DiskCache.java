...
public class DiskCache implements ICache {
    ...
    public DiskCache(Context context) {
        this(context, getDiskCacheDir(context, SuperConfig.CACHE_DISK_DIR),
                calculateDiskCacheSize(getDiskCacheDir(context, SuperConfig.CACHE_DISK_DIR)));
    }
    ...
    public DiskCache(Context context, File diskDir, long diskMaxSize) {
        ...
        try {
            cache = DiskLruCache.open(diskDir, CommonUtil.getVersionCode(context), 1, diskMaxSize);
        } catch (IOException e) {
            e.printStackTrace();
            ...
            if (snapshot != null) {
                ...
                if (!TextUtils.isEmpty(content)) {
                    ...
                    if ((createTime + expireMills > Calendar.getInstance().getTimeInMillis())
                            || expireMills == SuperConfig.CACHE_NEVER_EXPIRE) {
    ...
    private String getMd5Key(String key) {
        return MD5Util.byteArrayToHexString(key.getBytes());
    }
